...
export class MagickState {
				...
				ImageMagick.read(this.sourceBytes!, (image) => {
					try {
						...
						if ((this.settings.extentW ?? 0) > 0 || (this.settings.extentH ?? 0) > 0) {
							...
							image.extent(this.settings.extentW!, this.settings.extentH!, Gravity[gravityKey]);
	...
	handleDownload(
		...
		nameParts.pop();
		this.processedImageName = `${nameParts.join('.')}-edited.${this.processedImageFormat}`;
		...
		const percentageChange = (
			((blob.size - this.originalImageSize) / this.originalImageSize) *
			100
		).toFixed(1);
		const sizeChangeStr = `${newSizeKB} KB (${Number(percentageChange) > 0 ? '+' : ''}${percentageChange}%)`;
