import {
    ...
    MagickFormat,
    Percentage
} from 'https://cdn.jsdelivr.net/npm/@imagemagick/magick-wasm@0.0.21/+esm';
...
async function main() {
    ...
    try {
        const wasmUrl =
            "https://cdn.jsdelivr.net/npm/@imagemagick/magick-wasm@0.0.21/dist/magick.wasm";
        ...
        uploadText.textContent = "Error: Could not load image library.";
...
async function convertImages() {
    ...
    for (const fileEntry of imageFiles) {
        ...
        // Wrap ImageMagick operations in a Promise for async handling
        await new Promise((resolve, reject) => {
            (async () => {
                try {
                    const fileBytes = new Uint8Array(await fileEntry.originalFile.arrayBuffer());

                    await ImageMagick.read(fileBytes, (image) => {
                        // Apply conversion settings to the image
                        applyMagickSettings(image, settings);
                        
                        const outputFormat = getMagickFormat(settings.outputFormat);
...
function applyMagickSettings(image, settings) {
    ...
    if (settings.outputFormat === 'png') {
        image.define("png:compression-level", settings.pngCompression.toString());
    }
...
function showSlide(index) {
    const slides = document.querySelectorAll(
        "#1carouselContainer .carousel-slide"
    );
